# ISO 3166-1 alpha-2 country codes and names. Not backed by a table since
# this is static reference data — used for the profile country picker and
# for deriving a user's flag emoji.
class Country
  LIST = {
  ...
  def self.options_for_select
    LIST.sort_by { |_code, name| name }.map { |code, name| [ name, code ] }
  end

  def self.name_for(code)
    LIST[code.to_s.upcase]
  end

  # Two-letter ISO code -> flag emoji, via Unicode Regional Indicator
  # Symbols (each letter maps to a codepoint offset from U+1F1E6).
  def self.flag_for(code)
    return nil if code.blank? || !LIST.key?(code.to_s.upcase)

    code.to_s.upcase.each_char.map { |char| (0x1F1E6 + (char.ord - "A".ord)) }.pack("U*")
  end
end
